...
def visualization(sess, model, data_loader, filename):
	...
	if ckpt and ckpt.model_checkpoint_path:
		...
		print(" [!] loading parameters failed  ...")
		return
	user,item, label, u_texts, i_texts, idx = data_loader.sample_point()

	feed_dict = {model.u_input: user,
				model.i_input: item,
				model.label: label,
				model.u_text: u_texts,
				model.i_text: i_texts,
				model.training: False}
	res = sess.run([model.word_user_alpha, model.word_item_alpha, model.doc_user_alpha, model.doc_user_alpha], feed_dict = feed_dict)
	...
	# 	else:
	# 		print(item.shape)
	visual(res, data_loader, idx, filename)
